...
export default async function DugoutPage({ searchParams }: Props) {
  ...
  const primaryTeamDivision: MLBDivisionStandings | null = (() => {
    ...
    return allStandings.find(div =>
      div.teams.some(t => t.name === primaryTeam.name || t.abbreviation === primaryTeam.abbr)
    ) ?? null
  })()
  ...
  const teamKeywords = [primaryTeam?.name ?? '', primaryTeam?.short ?? '', primaryTeam?.abbr ?? '']
          ...
          <div>
            ...
            <div className="bg-white border border-stone-200 overflow-hidden">
              {!primaryTeamDivision ? (
                ...
                const isPrimary = team.name === primaryTeam?.name || team.abbreviation === primaryTeam?.abbr
