 ...
 * Left 1/3: persistent News Wire (news + injury report + transaction wire)
 * Right 2/3: Streamers, Sell/Sit, Sleepers, Two-Start, Platform Scoring
 */
...
function ReadLine({ children }: { children: React.ReactNode }) {
  return (
    <p className="font-serif italic text-xs text-stone-500 mb-3 leading-relaxed">{children}</p>
  )
}

// ── LEFT COLUMN: News Wire ────────────────────────────────────────────────────
...
function NewsWire({
  ...
  return (
    <div className="flex flex-col gap-0 sticky top-4">

      {/* Section switcher */}
      <div className="flex border-b border-stone-200 mb-4">
        {[
          { key: 'news' as const,         label: 'News' },
          { key: 'injuries' as const,     label: 'IL' },
          { key: 'transactions' as const, label: 'Wire' },
        ].map(tab => (
          <button
            key={tab.key}
      ...
      {activeSection === 'news' && (
        <div>
          <SectionLabel live>News Wire</SectionLabel>
          ...
          <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
            {filteredNews.length === 0 ? (
              <div className="px-4 py-8 text-center font-serif italic text-stone-400 text-sm">
                ...
                {filteredNews.slice(0, 15).map((item, i) => {
                  ...
                  return (
                    <a
                      ...
                      className="flex items-start gap-2.5 px-4 py-3 hover:bg-stone-50 transition-colors"
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-serif text-xs font-semibold text-stone-900 leading-snug line-clamp-2">
                          {item.title}
                        </div>
            ...
            <div className="px-4 py-2.5 border-t border-stone-100 text-center bg-stone-50">
              <Link
                href="/fantasy/news"
                className="font-mono text-[9px] uppercase tracking-widest text-orange-500 hover:text-orange-600"
              >
                Full wire → {news.length} stories
              </Link>
            </div>
          </div>
        </div>
      ...
      {activeSection === 'injuries' && (
        <div>
          ...
          <ReadLine>Active IL placements across all 30 teams — last 30 days.</ReadLine>
          <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
            {ilList.length === 0 ? (
              <div className="px-4 py-8 text-center font-serif italic text-stone-400 text-sm">
  ...
  />
                        ...
                        {t.injury_reason && (
                          <div className="font-serif italic text-[11px] text-stone-400">
                            {t.injury_reason}
                          </div>
                        )}
                        <div className="font-mono text-[9px] text-stone-300 mt-0.5">
            ...
            <div className="px-4 py-2.5 border-t border-stone-100 text-center bg-stone-50">
              <span className="font-mono text-[9px] uppercase tracking-widest text-stone-400">
                {ilList.length} active placements
              </span>
            </div>
          </div>
        </div>
      ...
      {activeSection === 'transactions' && (
        <div>
          <SectionLabel>Transaction Wire</SectionLabel>
          <ReadLine>Callups, options, DFAs, and trades across all teams — last 14 days.</ReadLine>
          <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
            {transactions.length === 0 ? (
              ...
              <div className="divide-y divide-stone-50">
                {transactions.slice(0, 25).map((t, i) => {
                  ...
                  const affiliate = t.from_affiliate_level || t.to_affiliate_level
                  const teamName = t.from_team_name || t.to_team_name
                  return (
                    <div key={i} className="px-4 py-3 flex items-start gap-2.5">
                      <span
  ...
  {t.team_id != null ? (
    ...
    <span className="font-mono text-[9px] px-1.5 py-0.5 rounded bg-stone-100 text-stone-500">
      {t.team_name.split(' ').slice(-1)[0]}
    </span>
  )}
</div>
                        {teamName && (
                          <div className="font-mono text-[9px] text-stone-400 mt-0.5">
                            {t.category === 'CALLUP' ? `from ${teamName}` : `to ${teamName}`}
                            {affiliate && ` (${affiliate})`}
                          </div>
                        )}
                        <div className="font-mono text-[9px] text-stone-300 mt-0.5">
            ...
            <div className="px-4 py-2.5 border-t border-stone-100 text-center bg-stone-50">
              <span className="font-mono text-[9px] uppercase tracking-widest text-stone-400">
                {transactions.length} moves in last 14 days
              </span>
            </div>
          </div>
        </div>
  ...
  if (isTop) {
    return (
      <div
        ...
        style={{ background: '#1A1A1A', border: '0.5px solid rgba(255,87,34,0.3)' }}
      >
        <div className="px-5 py-4">
          ...
          {pick.details && (
            <div
              className="mt-3 pt-3 flex gap-4 flex-wrap"
    ...
    <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
      ...
      {picks.map((pick, i) => {
        ...
        return (
          <div
            key={i}
            className="grid px-4 py-3 border-b border-stone-50 last:border-0 items-center"
            style={{ gridTemplateColumns: '1fr 90px 90px 50px' }}
          >
            <div>
...
function PlatformTable({ picks }: { picks: FantasyPick[] }) {
  const streamers = picks.filter(p => p.pick_type === 'streamer' || (p.signal_score ?? 0) >= 50).slice(0, 5)
...
export default function FantasyDashboard({
 ...
 const allPicks: FantasyPick[] = [
  ...
  // Two-start picks are tagged with pick_type === 'twostart' or from the streamer list with two_start details
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 pb-16">
      ...
      <div className="py-8 border-b border-stone-200 mb-8">
        <div className="font-mono text-[9px] uppercase tracking-widest text-orange-500 mb-2">
          ⊕ The Edge · Fantasy Desk · Pro
        </div>
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            <h1 className="font-serif text-4xl md:text-5xl font-bold text-stone-900 leading-none tracking-tight">
              The Trading Floor<span className="text-orange-500">.</span>
            </h1>
            <p className="font-serif italic text-stone-500 mt-2 text-sm max-w-xl">
              News, injuries, transactions, and the model's calls — everything that moves your roster tonight.
            </p>
          </div>
          <div className="flex gap-3 flex-wrap">
             {[
              ...
              <div key={s.label} className="text-center px-3 py-2 bg-stone-50 rounded-lg border border-stone-200">
                <div className="font-mono text-lg font-bold leading-none" style={{ color: s.color }}>
                  {s.count}
                </div>
        ...
        {isStale && (
          <div className="mt-4 font-mono text-[10px] text-amber-700 bg-amber-50 border border-amber-200 px-3 py-2 rounded-lg">
      ...
      <div className="space-y-10">
          ...
          <section>
            ...
            <ReadLine>
              Best arms to start tonight — ranked by matchup quality, park factor, and underlying Statcast metrics. The data makes the case; you make the call.
            </ReadLine>
            {picks.streamer.length === 0 ? (
              <div className="bg-white border border-stone-200 rounded-xl px-4 py-8 text-center font-serif italic text-stone-400 text-sm">
          ...
          {picks.faller.length > 0 && (
            <section>
              ...
              <ReadLine>
                Elite arms in the wrong spot — park, matchup, or trajectory working against them. The underlying data, not the name on the jersey.
              </ReadLine>
          ...
          {picks.sleeper.length > 0 && (
            <section>
              ...
              <ReadLine>
                FIP significantly better than ERA. The model likes these more than their surface numbers suggest — and likely more than the waiver wire does.
              </ReadLine>
          ...
          <section>
            ...
            <ReadLine>
              Pitchers going twice this week, graded on both matchups combined. Start 1 and Start 2 quality rated independently.
            </ReadLine>
          ...
          <section>
            ...
            <ReadLine>
              Same projection, four scoring systems. Pick the right arm for your format — DraftKings rewards Ks more, ESPN rewards wins more.
            </ReadLine>
