...
function FactorBar({ name, score, label }: { name: string; score: number; label: string }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2 border-b border-stone-100 last:border-0">
      ...
      <div className="flex-1 h-[4px] bg-stone-100 rounded-full overflow-hidden relative">
        <div
      ...
      <div className="max-w-2xl mx-auto px-6 sm:px-8 py-10 sm:py-14">
        ...
        {tab === 'compare' && (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-stone-200 rounded-xl overflow-hidden shadow-sm">
              <div className="bg-[#1A1A1A] flex items-start justify-between p-4 sm:p-5">
                <div>
                  <p className="font-mono text-[9px] uppercase tracking-widest text-stone-400 mb-1">The Edge · Today&apos;s brief</p>
              ...
              <div className="bg-[#1A1A1A] px-4 sm:px-5 pb-4 sm:pb-5">
              ...
              <div className="bg-white px-4 sm:px-5 pt-4 pb-5">
            ...
            <div className="p-6 bg-[#1A1A1A] rounded-xl text-center shadow-sm">
          ...
          <div className="space-y-6">
            ...
            <div className="overflow-hidden border border-stone-200 rounded-xl shadow-sm bg-white">
              ...
              <div className="grid grid-cols-2">
                <div className="px-4 py-3 bg-stone-50 border-r border-stone-200 border-b">
                  <p className="font-mono text-[9px] uppercase tracking-widest text-stone-500 font-bold">AI Overview</p>
                </div>
                <div className="px-4 py-3 bg-[#FF5722]/10 border-b border-[#FF5722]/20">
              ...
              {GAPS.map((g, i) => (
                <div key={i} className={`grid grid-cols-2 ${i !== GAPS.length - 1 ? 'border-b border-stone-100' : ''}`}>
                  <div className="px-4 py-4 border-r border-stone-200 bg-white">
        ...
        {tab === 'archive' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 divide-x divide-stone-100 bg-white border border-stone-200 rounded-xl shadow-sm overflow-hidden">
              {[
                ...
                <div key={s.val} className="p-5 text-center bg-white flex flex-col justify-center">
                  <p className="font-black text-3xl text-[#1A1A1A] leading-none" style={{ fontFamily: 'Fraunces, serif' }}>
                    {s.val}
                  </p>
                  <p className="font-mono text-[9px] font-bold uppercase tracking-widest text-[#FF5722] mt-2 leading-snug">
                    {s.lbl}
                  </p>
                  <p className="font-mono text-[8px] text-stone-400 mt-1">
                    {s.sub}
                  </p>
                </div>
              ))}
            ...
            {/* Recent analyses */}
            <div>
              <p className="font-mono text-[10px] uppercase tracking-widest text-stone-400 mb-3 mt-8">Recent Analyses</p>
              <div className="border border-stone-200 rounded-xl overflow-hidden shadow-sm bg-white">
                {READS.map((r, i) => (
                  <div
                    key={i}
                    className={`flex flex-col sm:flex-row sm:items-center px-5 py-4 ${i !== READS.length - 1 ? 'border-b border-stone-100' : ''}`}
                  >
                    <div className="flex-1 min-w-0 mb-3 sm:mb-0 sm:mr-4">
                      <p className="font-mono text-[11px] font-bold text-stone-900">{r.matchup}</p>
                      <p className="font-serif italic text-[12px] text-stone-500 mt-1">{r.note}</p>
                    </div>
                    <div className="flex justify-between items-center sm:block sm:text-right shrink-0 bg-stone-50 sm:bg-transparent p-3 sm:p-0 rounded border border-stone-100 sm:border-transparent">
                      ...
                      <p className="font-mono text-[9px] uppercase tracking-widest text-stone-400 sm:mt-1.5">
            ...
            <div className="p-6 border border-stone-200 bg-white rounded-xl shadow-sm mt-8">
        ...
        <div className="text-center pb-8">
          <p className="font-black text-3xl sm:text-4xl text-[#1A1A1A] mb-3" style={{ fontFamily: 'Fraunces, serif' }}>
            Free to read. Pro for absolute depth.
          </p>
          <p className="font-mono text-[11px] text-stone-400 mb-8 leading-relaxed">
            Daily MLB brief · Historical Archive · Statistical analysis only
          </p>
          <Link
            href="/mlb"
            className="inline-block bg-[#FF5722] text-white px-8 py-3.5 font-mono text-[11px] font-bold uppercase tracking-widest hover:bg-[#E64E1E] transition-colors rounded-sm"
          >
