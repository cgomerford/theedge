...
import { getLast7DaysPitcherWorkload, type Last7DaysWorkload } from '@/lib/pitcher-workload'
import type { FielderAlignmentEntry } from '@/components/FieldingAlignmentDiamond'
import {
  buildScoutReport,
  type ScoutInputs,
  type ScoutReport,
...
import { getPitcherHandedness } from '@/lib/mlb'
...
export type ScoutReportBundle = {
  ...
  awayPitcherStats: { era: number | null; whip: number | null; k_per_9: number | null; bb_per_9: number | null; l3_era: number | null; fip: number | null } | null
  homePitcherStats: { era: number | null; whip: number | null; k_per_9: number | null; bb_per_9: number | null; l3_era: number | null; fip: number | null } | null
  ...
  awayBullpen: ScoutInputs['awayBullpen']
  homeBullpen: ScoutInputs['homeBullpen']
  awayHotStreaks: HotStreakPlayer[]
  homeHotStreaks: HotStreakPlayer[]
  awayLiteralBatters: Awaited<ReturnType<typeof getTopBatterStreaks>>['all']
  homeLiteralBatters: Awaited<ReturnType<typeof getTopBatterStreaks>>['all']
  awayPitcherTrend: Awaited<ReturnType<typeof getPitcherTrend>>
  homePitcherTrend: Awaited<ReturnType<typeof getPitcherTrend>>
  awayLineupBatters: import('@/lib/lineups').LineupBatter[]
  homeLineupBatters: import('@/lib/lineups').LineupBatter[]
...
export async function getScoutReportBundle(gamePk: number): Promise<ScoutReportBundle | null> {
  try {
    ...
    // awayStreakData/homeStreakData and the pitcher-trend calls below were
    // previously fetched-then-discarded here (parity with page.tsx's shape,
    // no consumer at the time). GamePreviewTeaser now reads both directly
    // off the bundle's return value — see .all/.awayLiteralBatters below —
    // so these are no longer wasted work, just returned instead of voided.
    ...
    // Cheap — a single Supabase read of player_fielding_run_value joined
    // against the confirmed/projected lineup, same call page.tsx already
    // makes for the live Scout Report page. Not a live external-API walk.
    const seasonYearForFielding = new Date().getFullYear()
    const [awayFieldingAlignment, homeFieldingAlignment] = await Promise.all([
      getFieldingAlignment(awayLineup?.batters ?? [], seasonYearForFielding),
      getFieldingAlignment(homeLineup?.batters ?? [], seasonYearForFielding),
    ])
    const [
  ...
  const [awayFullStats, homeFullStats, awayPitcherThrowsFetched, homePitcherThrowsFetched] = await Promise.all([
    ...
    awayPitcherId ? getPitcherHandedness(awayPitcherId) : Promise.resolve(null),
    homePitcherId ? getPitcherHandedness(homePitcherId) : Promise.resolve(null),
  ])
    ...
    const [awayPitcherHotZones, homePitcherHotZones, awayPitcherArsenalZones, homePitcherArsenalZones] = await Promise.all([
     ...
     const awayPitcherThrows = (awayPitcherThrowsFetched ?? 'R') as 'L' | 'R'
  const homePitcherThrows = (homePitcherThrowsFetched ?? 'R') as 'L' | 'R'
     // TTOFatigueChart reads *_avg (not *_woba) as of 2026-08-20 — page.tsx
    // carries both, this bundle previously only had wOBA which is why the
    // chart rendered on the game page but showed "Not enough starts" in the
    // video preview. Both fields are already on awayFullStats from
    // pitcher-full-stats.ts, so this is a no-cost fix.
    ...
    // Per-reliever pitch counts over the last 7 days — for the bullpen-bars
    // slide in GamePreviewTeaser. L7 window only, cached 30min (see
    // pitcher-workload.ts line 78) — safe cost profile, same tier as
    // getBullpenData above.
      const [awayWorkloadRaw, homeWorkloadRaw] = await Promise.all([
      getLast7DaysPitcherWorkload(game.teams.away.team.id),
      getLast7DaysPitcherWorkload(game.teams.home.team.id),
    ])

    // Prepend tonight's SP to the workload list so the bullpen slide leads
    // with the starter. If the SP happens to already appear in the returned
    // pitchers array (e.g. they've pitched in the last 7 days), we take the
    // real row (has actual byDate counts) rather than a duplicate stub. We
    // tag the SP row with a __isStarter marker for the UI to style/label —
    // Last7DaysWorkload's PitcherWorkloadRow type doesn't have this field
    // natively, but the extra key is preserved through the plain object
    // shape and the UI reads it via optional chaining.
    function prependSP(
      raw: Awaited<ReturnType<typeof getLast7DaysPitcherWorkload>>,
      pitcherId: number | undefined,
      pitcherName: string | undefined,
    ) {
      if (!pitcherId || !pitcherName) return raw
      const existing = raw.pitchers.find(p => p.playerId === pitcherId)
      if (existing) {
        // Move existing SP row to the front, tagged as starter
        const rest = raw.pitchers.filter(p => p.playerId !== pitcherId)
        return { ...raw, pitchers: [{ ...existing, __isStarter: true } as any, ...rest] }
      }
      // SP hasn't pitched in the last 7 days — synthesize an empty row.
      const emptyByDate: Record<string, number> = {}
      for (const d of raw.dates) emptyByDate[d] = 0
      const stub = {
        playerId: pitcherId,
      ...
      return { ...raw, pitchers: [stub as any, ...raw.pitchers] }
    }

    const awayWorkload = prependSP(awayWorkloadRaw, awayPitcherId, game.teams.away.probablePitcher?.fullName)
    const homeWorkload = prependSP(homeWorkloadRaw, homePitcherId, game.teams.home.probablePitcher?.fullName)
    ...
    const scoutInputs: ScoutInputs = {
      ...
      throws: awayPitcherThrowsFetched,
      ...
      throws: homePitcherThrowsFetched,
    ...
    return {
      ...
      homePitcherName: game.teams.home.probablePitcher?.fullName ?? 'TBD',
          awayPitcherId: awayPitcherId ?? null,
      homePitcherId: homePitcherId ?? null,
      // Small stat block for the SP Matchup slide — reads straight off
      // awayFullStats/homeFullStats (pitcher_stats Supabase table),
      // already fetched above via getPitcherStatsFull. Zero new fetch.
      awayPitcherStats: awayFullStats ? {
        era: (awayFullStats as any).era ?? null,
        whip: (awayFullStats as any).whip ?? null,
        k_per_9: (awayFullStats as any).k_per_9 ?? null,
        bb_per_9: (awayFullStats as any).bb_per_9 ?? null,
              l3_era: (awayFullStats as any).l3_era ?? null,
        fip: (awayFullStats as any).fip ?? null,
      } : null,
      homePitcherStats: homeFullStats ? {
        era: (homeFullStats as any).era ?? null,
        ...
        fip: (homeFullStats as any).fip ?? null,
      } : null,
      awayPitcherHotZones,
      homePitcherHotZones,
      ...
      awayBullpen: scoutInputs.awayBullpen,
      homeBullpen: scoutInputs.homeBullpen,
      awayHotStreaks: _awayHotStreaks,
      homeHotStreaks: _homeHotStreaks,
      awayLiteralBatters: awayStreakData.all,
      homeLiteralBatters: homeStreakData.all,
      awayPitcherTrend,
      homePitcherTrend,
      awayFieldingAlignment,
      homeFieldingAlignment,
      awayWorkload,
      homeWorkload,
      // Full lineup batter rows so GamePreviewTeaser can look up season
      // stats (avg/slg/ops/rbi) by player_id for the Streak Watch slide.
      awayLineupBatters: awayLineup?.batters ?? [],
      homeLineupBatters: homeLineup?.batters ?? [],
    }
  } catch (err) {
    console.error(`getScoutReportBundle failed for gamePk ${gamePk}:`, err)
    return null
  }
}