...
// Visible fallback instead of a silently-hidden broken image — shows the
// team's abbreviation on a colored circle so a logo failure is obvious
// and legible rather than a blank white void.
...
export default function TeamDugoutView({
  ...
  return (
    <div style={{ fontFamily: "'JetBrains Mono', monospace", background: '#FAF8F3', maxWidth: 1280, margin: '0 auto', padding: '24px 16px 64px' }}>
      ...
      <div style={{ background: team.primary_color, borderRadius: 14, padding: '28px 32px', marginBottom: 16, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20 }} className="dugout-main-layout">
        <div>
          <ExpandableSchedule schedule={schedule} />

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }} className="dugout-composition-grid">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 16, marginBottom: 16 }} className="dugout-moves-grid">
  ...
  <div key={tx.transaction_id} style={{ display: 'flex', gap: 10, padding: '11px 20px', borderBottom: i === Math.min(moves.length, 6) - 1 ? 'none' : '1px solid #f7f5ef', alignItems: 'center' }}>
    ...
    {/* Minor-league level badge — real field now that team-transactions.ts
        is in hand. is_milb_move + to_affiliate_level together tell us this
        was a callup/option involving a minor-league level, not just a
        major-league roster shuffle. */}
    ...
    <span style={{ fontSize: 12, color: '#1A1A1A' }}>{tx.player_name} — {tx.category}{tx.to_team_name ? ` to ${tx.to_team_name}` : ''}</span>
          ...
          <div style={{ background: '#fff', border: '1px solid #e7e2d8', borderRadius: 14, padding: 20, marginBottom: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }} className="dugout-rankings-grid">
              {rankings.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }} className="dugout-leaders-grid">
            <div style={{ background: '#fff', border: '1px solid #e7e2d8', borderRadius: 14, padding: 20 }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        @media (max-width: 900px) { .dugout-main-layout { grid-template-columns: 1fr !important; } }
        @media (max-width: 640px) {
          .dugout-composition-grid, .dugout-moves-grid, .dugout-leaders-grid, .dugout-minors-grid { grid-template-columns: 1fr !important; }
