...
type SeasonTrendPoint = { gameNumber: number; date: string; avg: number | null; obp: number | null; slg: number | null; ops: number | null; hr: number; rbi: number }
type PlottableMetric = 'avg' | 'obp' | 'slg' | 'ops' | 'hr' | 'rbi'
const METRICS: { key: PlottableMetric; label: string; format: (v: number) => string }[] = [
  ...
  { key: 'hr', label: 'HR', format: v => String(v) },
...
export default function SeasonTrendChart({ players }: { players: SelectedPlayer[] }) {
  const batters = players.filter(p => p.subjectType === 'batter')
  const [metric, setMetric] = useState<PlottableMetric>('avg')
  const [series, setSeries] = useState<Record<number, SeasonTrendPoint[]>>({})
  ...
  useEffect(() => {
    ...
    setLoading(true)
    Promise.all(batters.map(p =>
      fetch(`/api/lab/season-trend?id=${p.id}`)
        .then(r => r.json())
        .then(j => [p.id, j.points ?? [], j.teamId ?? null] as const)
    ))
      .then(entries => {
  ...
  const chartData = Array.from({ length: maxGames }, (_, i) => {
    ...
    for (const p of batters) {
      row[String(p.id)] = series[p.id]?.[i]?.[metric] ?? null
    }
    return row
  })
  ...
  // Renders nothing for every point except the player's last real game —
  // there the line ends in a circular, team-colored headshot instead of a
  // plain dot. Same "floating badge at line end" pattern as the team
  // rolling-average chart in LabDashboard.tsx, swapped from team logos to
  // player headshots.
  function makeDotRenderer(p: SelectedPlayer, color: string) {
    ...
    return (props: any) => {
      ...
      if (payload.gameNumber !== lastGameNumber || cx == null || cy == null) {
        return <g key={`empty-${p.id}-${payload.gameNumber}`} />
      }
      return (
        <g key={`avatar-${p.id}`}>
          <defs>
  ...
  return (
    <div className="border-t border-stone-200 pt-6">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div className="text-[10px] font-mono uppercase tracking-widest text-stone-400">Season trend — batters only</div>
        <div className="flex items-center gap-3">
          <CardExportToolbar targetRef={chartRef} fileName={`season-trend-${metric}-the-edge`} />
  ...
  formatter={(value) => {
    ...
    return typeof v === 'number' ? metricMeta.format(v) : '—'
  }}
  labelFormatter={l => `Game ${l}`}
/>
              <Legend
                formatter={(value: string) => {
                  const p = batters.find(b => String(b.id) === value)
                  return p ? p.fullName : value
                }}
