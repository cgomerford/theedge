// src/components/nfl/NflWrWeeklyHeatmap.tsx

import type { WrWeeklyGridRow } from '@/lib/nfl/queries'
...
export default function NflWrWeeklyHeatmap({ rows, maxWeeks = 18 }: { rows: WrWeeklyGridRow[]; maxWeeks?: number }) {
  if (rows.length === 0) {
    return (
  ...
  const allValues = rows.flatMap((r) => r.weeks.map((w) => w.avgSeparation).filter((v): v is number => v != null))
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid rgba(26,26,26,0.1)', padding: 20, overflowX: 'auto' }}>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: '#78716C', marginBottom: 12 }}>Cell = that week's average separation. Darker = more space created.</div>
      <table style={{ borderCollapse: 'collapse', fontFamily: "'JetBrains Mono', monospace", fontSize: 10 }}>
        <thead>
          <tr>
            <th style={{ textAlign: 'left', padding: '4px 8px', position: 'sticky', left: 0, background: '#fff' }}>WR</th>
            {weekNums.map((w) => <th key={w} style={{ padding: '4px 4px', color: '#A3A3A3', fontWeight: 400 }}>{w}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => {
            const byWeek = new Map(r.weeks.map((w) => [w.week, w.avgSeparation]))
            return (
              <tr key={r.playerId}>
                ...
                {weekNums.map((w) => {
                  ...
                  return <td key={w} title={v != null ? `${v.toFixed(1)} yds` : 'No data'} style={{ width: 26, height: 22, background: heatColor(v, min, max), border: '1px solid #F4F0E8' }} />
                })}
              </tr>
            )
          })}
