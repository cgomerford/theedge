...
// Four sections per pitcher:
//   1. Arsenal deep-dive (with grades)
//   2. Two-Strike Profile
//   3. Times Through the Order
//   4. First-Pitch Tendencies
        ...
        <div className="grid grid-cols-2 gap-px bg-stone-100 rounded-xl overflow-hidden">
          ...
          {entries.map(([pt, v]) => {
            ...
            return (
              <>
                {/* All counts bar */}
                <div key={`${pt}-all`} className="bg-white px-4 py-3 flex items-center gap-3">
                ...
                {/* Two-strike bar */}
                <div key={`${pt}-2k`} className="bg-white px-4 py-3 flex items-center gap-3">
                  <div className="flex-1 h-2 bg-stone-100 rounded-full overflow-hidden">
...
function TimesThrough({ stats }: { stats: PitcherStats | null }) {
 ...
 const buckets = [
  ...
  function qualityLabel(v: number | null): { label: string; color: string; bg: string } {
    ...
    if (v <= 0.360) return { label: 'Weakening', color: '#D97706', bg: 'rgba(217,119,6,0.08)' }
...
function PitcherBlock({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Arsenal */}
      <div>
        <h4 className="text-[10px] font-mono uppercase tracking-widest text-orange-600 font-bold mb-3">
          ⊕ Arsenal Breakdown
        </h4>
      ...
      <div>
        <h4 className="text-[10px] font-mono uppercase tracking-widest text-orange-600 font-bold mb-3">
          ⊕ Two-Strike Profile — What He Goes To With Two Strikes
        </h4>
      ...
      <div>
        <h4 className="text-[10px] font-mono uppercase tracking-widest text-orange-600 font-bold mb-3">
          ⊕ Times Through the Order — When to Get Him
        </h4>
